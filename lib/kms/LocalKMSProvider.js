...
export default class LocalKMSProvider extends KMSInterface {
  ...
  async sign(index, txHashHex) {
    // 1. Determine keyIndex from format (e.g. 'wallet:5' -> 5, or default to 0 for system)
    let baseIndex = 'system:master';
    let keyIndex = 0;

    if (index.startsWith('wallet:')) {
      const parts = index.split(':');
      keyIndex = parseInt(parts[1], 10);
    } else {
      baseIndex = index;
    }

    // Retrieve the private key — stays in this function scope only
    const bech32Key = await this.getKey(baseIndex);
    ...
    if (bech32Key.startsWith('xprv')) {
      ...
      // Derive the standard payment key: m / 1852' / 1815' / 0' / 0 / keyIndex
      ...
      const paymentKey = accountKey
        .derive(0)
        .derive(keyIndex); // Derive dynamic key index for the custodial wallet
