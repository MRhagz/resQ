import { MeshTxBuilder, resolveScriptHash, unixTimeToEnclosingSlot, deserializeAddress } from '@meshsdk/core';
import { systemWallet, provider, signAndSubmit, signAndSubmitMultiSig, getCustodialAddress } from './wallet.js';
...
export async function burnBatch(campaign, quantity) {
  ...
  txBuilder
    .mint(String(-quantity), policyId, assetNameHex)
    .mintingScript(claimForgeScript);
  ...
  if (campaign.startsAt && process.env.DEMO_MODE !== 'true') {
    ...
    txBuilder.invalidBefore(Number(startsAtSlot));
  }
  ...
  if (campaign.endsAt && process.env.DEMO_MODE !== 'true') {
    ...
    txBuilder.invalidHereafter(Number(endsAtSlot));
  }
