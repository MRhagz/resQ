...
export async function burnBatch(campaign, quantity) {
  ...
  // Set validity range: Burning must only happen DURING the distribution (plus 1 hour)
...
export async function runBurnPipeline() {
  ...
  // 2. Fetch disaster system_codes and duration parameters for asset naming & policies
  const disasterIds = [...new Set(pendingStubs.map(s => s.disaster_event_id))];
  const { data: disasters } = await supabase
    .from('disaster_events')
    .select('id, system_code, starts_at, ends_at')
    .in('id', disasterIds);
  ...
  for (const stub of pendingStubs) {
    ...
    try {
      console.log(`[Burn] Initiating multi-signature burn for stub ${stub.id}...`);
